    ...
    <div className="min-h-screen bg-navy-950">
      ...
      >
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          <p className="text-gold-400 text-xs font-display tracking-widest uppercase mb-3">Kingdom Investment</p>
          <h1 className="font-decorative text-3xl md:text-5xl gold-text mb-4">Give</h1>
          <p className="text-gold-300/60 font-body italic text-sm">
      ...
      <section className="bg-navy-900 py-14 px-4">
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
          {[
            ...
            <div key={title} className="card-navy rounded-sm p-6">
              <div className="text-3xl mb-3">{icon}</div>
              <h3 className="font-display text-gold-400 text-sm tracking-widest uppercase mb-2">{title}</h3>
      ...
      <section className="py-16 px-4">
        <div className="max-w-xl mx-auto">
          <div className="card-navy rounded-sm p-8">
            <p className="text-gold-400 text-xs font-display tracking-widest uppercase mb-6">Online Giving</p>
            ...
            <div className="flex gap-2 mb-6">
              {(['one-time', 'monthly'] as const).map(f => (
                <button
                  ...
                  className={`flex-1 py-2.5 text-xs font-display tracking-widest uppercase rounded-sm transition-colors ${
                    frequency === f ? 'bg-gold-600 text-navy-950' : 'border border-gold-700/40 text-gold-300 hover:border-gold-400'
                  }`}
                >
            ...
            <div className="grid grid-cols-5 gap-2 mb-4">
              {amounts.map(a => (
                <button
                  ...
                  className={`py-2.5 text-sm font-display tracking-wide rounded-sm transition-colors ${
                    amount === a && !custom ? 'bg-gold-600 text-navy-950' : 'border border-gold-700/40 text-gold-300 hover:border-gold-400'
                  }`}
                >
                  ${a}
                </button>
            ...
            <div className="mb-6">
              ...
              <select className="w-full bg-navy-950 border border-gold-700/30 rounded-sm px-4 py-3 text-gold-200 text-sm focus:outline-none focus:border-gold-500/60 transition-colors appearance-none">
            ...
            <button className="btn-gold w-full py-3.5 rounded-sm text-sm">
              {selected ? `Give $${selected}` : 'Proceed to Give'}
            </button>
          </div>

          {/* Bank Transfer */}
          <div className="mt-6 card-navy rounded-sm p-6">
            <p className="text-gold-400 text-xs font-display tracking-widest uppercase mb-4">Bank Transfer</p>
            <div className="space-y-2 text-sm text-gold-200/60">
              <p><span className="text-gold-400/80">Account Name:</span> Faith Believers Ministry Int'l</p>
              <p><span className="text-gold-400/80">Contact the church office for account details.</span></p>
            </div>
          </div>
        </div>
      </section>
